...
When in development we need to be able to test changes without affecting production systems (e.g. a database and connected services). One option is to simply not write to the production database[^1] in development. This can be easily accomplished adding a flag to the `skip` argument `target`s that upload assets, where the flag checks if an environmental variable is set to `production`:

[^1]: Where production database can be replaced with a `pins` board, or some other "container" of data assets
