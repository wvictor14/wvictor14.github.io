...
Quarto has slowly become one of my daily tools as a data scientist. Pretty much any analytical task involving data analysis - I turn to quarto instead of jupyter notebook or rmarkdown. More than that, quarto is not just for "notebook" analysis, but I have been using it to publish data-centric wiki-like websites. Quarto is well suited (on paper) for this type of product - reproducibility, integration with python, R, and web technologies etc.

But this post is about navigating some of the pains that comes along with quarto (and there are definitely some worth mentioning). Specifically, this is about speed and scalability, two features that I find rate-limiting in using quarto for large websites.
...
Though this works for standalone parameterized reports, it breaks in a Quarto website projects because website outputs lands in a shared `_site/` directory. It also doesn't handle the project-level dependencies those renders need: `_quarto.yml`, `.Rprofile`, `renv/`, `R/` helper functions, and shared data.

Following is a solution that is more generalizable
