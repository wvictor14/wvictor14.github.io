...
xy <- FetchData(
  seu,
  vars = c("umap_1", "umap_2", "seurat_annotations", rownames(seu))
)
```
...
bnch <- bench::mark(
  `Seurat, not sampled` = seu |>
    Seurat::FeaturePlot(features = features) |>
    plot(),
  iterations = 3,
  memory = FALSE
)
...
plots <- list(
  seu |>
    Seurat::FeaturePlot(features = c("CD8A", "FOXP3"), order = TRUE) &
    coord_equal(),
  seu_sampled |>
    Seurat::FeaturePlot(features = c("CD8A", "FOXP3"), order = TRUE) &
    coord_equal()
...
bnch_final <- bench::mark(
  `Seurat, not sampled` = Seurat::FeaturePlot(seu, features = features) |>
    plot(),
